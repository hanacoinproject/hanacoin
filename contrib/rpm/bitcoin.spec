...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 9502
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 9501
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 19502
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 19503
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 9502
	%{_sbindir}/semanage port -d -p tcp 9501
	%{_sbindir}/semanage port -d -p tcp 19502
	%{_sbindir}/semanage port -d -p tcp 19503
	for selinuxvariant in %{selinux_variants}; do
